...
import { pageWrap, pageTitle, pageSubtitle, pageHeader, inputStyle, skeletonStyle, thStyle, tdStyle } from "@/lib/styles";
      ...
      <div style={pageHeader}>
        <div>
          <h1 style={{ ...pageTitle, display:"flex", alignItems:"center", gap:10 }}>
            <ShieldAlert size={20} color="var(--red-light)"/> Spam Guard
          </h1>
            ...
            : shown.length===0 ? (
              ...
              <tr key={c.id}>
                <td style={tdStyle}>
                  ...
                  <div style={{fontSize:11,color:"var(--text-muted)",textTransform:"capitalize"}}>{c.platform}: {c.platform_id}</div>
                </td>
                <td style={tdStyle}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <div style={{width:64,height:5,background:"var(--bg-elevated)",borderRadius:99,overflow:"hidden",border:"1px solid var(--border)"}}>
                ...
                <td style={{...tdStyle,textAlign:"center"}}>
                  <button onClick={()=>toggleAI(c.id,c.ai_reply_enabled)} style={{ border:"none", cursor:"pointer", background:"none", padding:0 }}>
                    <span className={c.ai_reply_enabled ? "badge badge-green" : "badge badge-muted"}>
                      {c.ai_reply_enabled ? "Enabled" : "Disabled"}
                    </span>
                  </button>
                </td>
                <td style={{...tdStyle,textAlign:"center"}}>
                  <button onClick={()=>toggleVIP(c.id,c.is_vip)} style={{background:"var(--bg-elevated)",border:"1px solid var(--border)",cursor:"pointer",display:"inline-flex",alignItems:"center",padding:4,borderRadius:"var(--r-sm)"}}>
                    <Star size={16} style={{fill:c.is_vip?"var(--amber-light)":"none",color:c.is_vip?"var(--amber-light)":"var(--text-muted)"}}/>
                  </button>
                </td>
                <td style={{...tdStyle,textAlign:"right"}}>
                  <button onClick={()=>reset(c.id)} style={{background:"none",border:"none",cursor:"pointer",color:"var(--text-muted)",padding:4,display:"inline-flex",borderRadius:"var(--r-sm)"}}>
                    <Trash2 size={14}/>
                  </button>
                </td>
              </tr>
