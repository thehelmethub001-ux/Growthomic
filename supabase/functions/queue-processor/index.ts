    ...
    if (messageText?.startsWith("CMD_")) {
      ...
      // ══════════════════════════════════════
        ...
        if (!conversation.cart_state || conversation.cart_state.length === 0) {
          ...
          mediaUrls = undefined;
          mediaType = undefined;
          // Set a flag to force queue-processor to build items later
          (conversation as any)._bypassItemsLLM = true;
        }
      }
...
২. যে ছবিটি মিলেনি তার জন্য বিনীতভাবে জানান যে ওই নির্দিষ্ট মডেলটি বর্তমানে স্টকে নেই, তবে অন্য ছবিগুলোর নাম ও দাম সরাসরি জানিয়ে দিন।]` + "\n" + (messageText || "");
    ...
    if (aiResult.intent === "order_intent" && aiResult.orderData) {
      ...
      // ── CRITICAL FIX: Deterministic Items from Cart ──
      // If the user used the button flow, override AI hallucinated items with exact DB cart state
      if ((conversation as any)._bypassItemsLLM && conversation.cart_state && conversation.cart_state.length > 0) {
        orderData.items = [...conversation.cart_state];
        console.log(`[CHECKOUT] Overrode AI order items with deterministic cart state: ${orderData.items.length} items`);
      }
          ...
          try {
            ...
            if (realProduct) {
              ...
              if (item.variantId && realProduct.variations) {
                const variant = realProduct.variations.find((v: any) => String(v.id) === String(item.variantId) || String(v.woo_variation_id) === String(item.variantId));
                if (variant) {
      ...
      // Clear the cart_state if it was used for an order
      if ((conversation as any)._bypassItemsLLM || (conversation.cart_state && conversation.cart_state.length > 0)) {
        await getSupabaseClient().from("conversations").update({ cart_state: [] }).eq("id", conversation.id);
        console.log(`[CHECKOUT] Cleared cart_state for conversation ${conversation.id}`);
      }
    ...
    // ── Step 8.8.5: Size-triggered color carousel (for free-text size requests)
    // If the customer types a size (e.g. "xl lagbe") and Gemini responds,
    // we also send a carousel of ALL colors available in that size for the current product.
    const sizeMatch = !isImageMessage ? messageText.match(/\b(s|m|l|xl|xxl|xxxl)\b/i) : null;
    const contextProdId = aiResult.detectedProductId || conversation.lastProductId;
    if (sizeMatch && contextProdId && aiResult.intent !== "order_intent" && !messageText.startsWith("CMD_")) {
      const requestedSize = sizeMatch[1].toUpperCase();
      try {
        const { data: sizeProduct } = await sb.from("products").select("*").eq("id", contextProdId).single();
        if (sizeProduct && sizeProduct.variations) {
          const inStockVariants = (sizeProduct.variations || []).filter((v: any) => (v.stock_quantity ?? 0) > 0);
          
          const colorsWithSize = new Map<string, { price: number; imageUrl: string }>();
          for (const v of inStockVariants) {
            if (v.attributes?.Size?.toUpperCase() === requestedSize) {
              const color = v.attributes?.Color || "Default";
              if (!colorsWithSize.has(color)) {
                const price = v.sale_price || v.regular_price || sizeProduct.sale_price || sizeProduct.regular_price;
                const imageUrl = v.image_url || (sizeProduct.images && sizeProduct.images[0]) || "";
                colorsWithSize.set(color, { price, imageUrl });
              }
            }
          }

          const uniqueSizeColors = Array.from(colorsWithSize.entries());
          // Only show carousel if there are multiple options, OR if it's a generic inquiry
          if (uniqueSizeColors.length > 0) {
            console.log(`[SIZE-CAROUSEL] Showing ${uniqueSizeColors.length} color(s) in size ${requestedSize} for product ${sizeProduct.name}`);
            
            if (platform === "whatsapp") {
              const { sendWhatsAppInteractiveList } = await import("../_shared/platform-send.ts");
              const rows = uniqueSizeColors.map(([colorName, info]) => ({
                id: `CMD_SELECT_COLOR:${sizeProduct.id}:${colorName}`,
                title: colorName.slice(0, 24),
                description: `৳${info.price}`,
              }));
              await sendWhatsAppInteractiveList(
                platformId,
                `স্যার, ${requestedSize} সাইজে আমাদের কাছে এই কালারগুলো available আছে:`,
                "কালার বেছে নিন",
                [{ title: "Available Colors", rows }]
              );
            } else {
              const { sendCarouselMessage } = await import("../_shared/platform-send.ts");
              const fallbackImg = (sizeProduct.images && sizeProduct.images[0]) || "";

              let colorPriceText = `স্যার, ${requestedSize} সাইজে এই কালারগুলো available আছে:\n\n`;
              uniqueSizeColors.forEach(([colorName, info]) => {
                colorPriceText += `🎨 ${colorName} — ৳${info.price}\n`;
              });
              colorPriceText += "\nযেটা নেবেন সেটার কার্ডে ট্যাপ করুন 👇";
              await sendTextMessage(platform as Platform, platformId, colorPriceText);

              const elements = uniqueSizeColors.map(([colorName, info]) => ({
                title: colorName.slice(0, 80),
                subtitle: `৳${info.price}`,
                imageUrl: info.imageUrl || fallbackImg,
                buttonTitle: "এই কালারটা নেবো",
                buttonPayload: `CMD_SELECT_COLOR:${sizeProduct.id}:${colorName}`,
              }));
              await sendCarouselMessage(platform as "messenger" | "instagram", platformId, elements);
            }
          }
        }
      } catch (sizeCarouselErr) {
        console.error("Size-triggered carousel failed:", sizeCarouselErr);
      }
    }
